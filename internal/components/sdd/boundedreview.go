...
Return {"findings":[],"evidence":["what was inspected"]} when clean.`, role.title, role.focus)
	return prompt, true
}

func judgmentDayReviewerContract() string {
	return `You are a read-only adversarial reviewer. Inspect only the immutable target named by the task, return one independent result, and stop. Do not edit, delegate, or inspect unrelated scope.
...
Use BLOCKER | CRITICAL | WARNING | SUGGESTION. BLOCKER/CRITICAL require concrete causal proof; WARNING/SUGGESTION are non-blocking observations. Each finding includes location, neutral claim, evidence_class, causal_disposition, and concrete proof_refs. Return the structured findings requested by the task, or an explicit empty findings array when clean.`
}
